...
def plotResult(file):
    #with open(file + '_conn0_run4.csv', newline="", encoding="ISO-8859-1") as filecsv:
       ...
       # plt.plot(x, ym)
        ...
        plt.title('singolo incrocio reward queue (alpha 0.1 gamma 0.99) misura total waiting time')
        plt.xlabel('step')
        ...
        plt.xlabel("step")
        plt.title("singolo incrocio reward queue (alpha 0.1 gamma 0.99) misura total stopped ")
        ...
        capsize=4)
        plt.xlabel("step")
        plt.title("singolo incrocio reward queue(alpha 0.1 gamma 0.99) misura mean waiting time ")
        ...
        capsize=4)
        plt.xlabel("step")
        plt.title("singolo incrocio queue (alpha 0.1 gamma 0.99) misura mean speed")
        plt.ylabel("system mean speed(Km/h)")
        plt.show()
if __name__ == '__main__':
    plotResult('D:/programmi/sumo/esperimenti semafori/outputs/2way-single-intersection/result-alpha0.99-gamma0.1 wait test')